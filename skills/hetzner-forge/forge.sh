...
build_user_data() {
  ...
  if [ -n "$WL_INSTALL" ]; then
    ...
    printf 'ADMIN_USER=%q OPENCODE_API_KEY=%q OPENCODE_PORT=%q OPENCODE_USER=%q OPENCODE_PASSWORD=%q' \
      "$ADMIN_USER" "$OPENCODE_API_KEY" "$OPENCODE_PORT" "$OPENCODE_USER" "$OPENCODE_PASSWORD"
