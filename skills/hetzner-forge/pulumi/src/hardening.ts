...
export function buildCloudInit(
  ...
  const ocKey = cfg.opencodeApiKey ?? pulumi.output("");
  const ocPass = cfg.opencodePassword ?? pulumi.output("");
  return pulumi.all([hardenEnv(cfg, w), ocKey, ocPass]).apply(([envStr, key, pass]) => {
    const parts: string[] = [
      "#cloud-config",
      "package_update: true",
    ...
    if (w.install) {
      let instEnv = `ADMIN_USER=${shq(cfg.adminUser)} `;
      if (key) {
        instEnv +=
          `OPENCODE_API_KEY=${shq(key)} OPENCODE_PORT=${shq(String(cfg.opencodePort))} ` +
          `OPENCODE_USER=${shq(cfg.opencodeUser)} OPENCODE_PASSWORD=${shq(pass)} ` +
          `OPENCODE_MODEL=${shq(cfg.opencodeModel)} `;
      }
      parts.push(
        `  - ${instEnv}bash /opt/hetzner-forge/install-workload.sh 2>&1 | tee /var/log/hetzner-forge-install.log`,
      );
    }
